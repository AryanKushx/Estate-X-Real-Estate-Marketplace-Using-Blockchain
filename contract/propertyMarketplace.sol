...
contract PropertyMarketplace {
    ...
    function getAllowance(address spender, uint _index) public view returns (uint) {
        return properties[_index].houseToken.allowance(properties[_index].owner, spender);
    }
    ...
    function getBalance(uint index, address owner) public view returns(uint) {
        return properties[index].houseToken.balanceOf(owner);
    }
    ...
    function getPropertyPricePerToken(uint _index) public view returns(uint) {
        return properties[_index].stockData.price / properties[_index].stockData.numShares;
    }

    function getPropertyPrice(uint _index) public view returns(uint) {
        return properties[_index].stockData.price;
    }

    function getPropertyTotalTokens(uint _index) public view returns(uint) {
        return properties[_index].stockData.numShares;
    }

    function getPropertyTokensRemaining(uint _index) public view returns(uint) {
        return properties[_index].stockData.numShares - properties[_index].stockData.sold;
    }

    function canBeCancelled(uint _index) public view returns(bool) {
        return (
            msg.sender == properties[_index].owner ||
            properties[_index].stockData.sold == 0 ||
            properties[_index].status != Status.SaleCancelled
    ...
    function canUpdatePrice(uint _index) public view returns(bool) {
        return (
            msg.sender == properties[_index].owner ||
            properties[_index].stockData.sold == 0 ||
            properties[_index].status != Status.SaleCancelled
    ...
    function canUpdateShares(uint _index) public view returns(bool) {
        return (
            msg.sender == properties[_index].owner ||
            properties[_index].stockData.sold == 0 ||
            properties[_index].status != Status.SaleCancelled
    ...
    function updatePropertyPrice(uint _index, uint _price) public {
        require(msg.sender == properties[_index].owner, "Only the property owner can cancel the sale");
        require(properties[_index].stockData.sold == 0, "The sale cannot be updated if some tokens are already issued");
        require(properties[_index].status != Status.SaleCancelled, "The property sale is cancelled");
        properties[_index].stockData.price = _price;
    ...
    function cancelPropertySale(uint _index) public {
        require(msg.sender == properties[_index].owner, "Only the property owner can cancel the sale");
        require(properties[_index].stockData.sold == 0, "The sale cannot be cancelled if some tokens are already issued");
        properties[_index].status = Status.SaleCancelled;
    }

    function updatePropertyShares(uint _index, uint _shares) public {
        require(msg.sender == properties[_index].owner, "Only the property owner can cancel the sale");
        require(properties[_index].stockData.sold == 0, "The sale cannot be updated if some tokens are already issued");
        require(properties[_index].status != Status.SaleCancelled, "The property sale is cancelled");
        properties[_index].stockData.numShares = _shares;
    ...
    function buyProperty(uint _index) public payable  {
        ...
        require(
            properties[_index].stockData.sold < properties[_index].stockData.numShares ||
            properties[_index].status != Status.SoldOut,
            "All shares of this property are sold out."
        );
        ...
        // Transfer property token to buyer
        require(properties[_index].houseToken.transfer(msg.sender, 1 ether), 
        "Transfer of house tokens from app to buyer failed");
    ...
    // New deleteProperty function
    function deleteProperty(uint _index) public {
        require(msg.sender == properties[_index].owner, "Only the property owner can delete the property");
        require(properties[_index].status == Status.OnSale, "Property sale must be ongoing to delete");
